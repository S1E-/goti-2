...
from configurations.database import result_sent_record
...
def get_latest_result(update, context, collection=None, bs_result_and_admitcard=None):
    ...
    unordered_list_list = bs.find('div', {'id': 'box1', 'align': 'left'}).find(
    ...
    def generator(chat_id, u_list, query_time):
        ...
        past_exams = result_sent_record.find(
    ...
    if update is not None:
        context.bot.send_chat_action(chat_id, action=ChatAction.TYPING)
    results_data = list(generator(chat_id, unordered_list_list, query_time))

    if len(results_data) != 0:

        context.bot.send_chat_action(chat_id, action=ChatAction.TYPING)

        #context.bot.send_message(chat_id, text="<u><b>RESULTS</b></u> 📈 \n\n\n{0}\n\n\n<em>This is all we have for now. Enjoy! 🥳</em>".format('\n\n'.join(results_data)), disable_web_page_preview=1)
        context.bot.send_message(chat_id, text="<u><b>RESULTS</b></u> 📈 \n\n{0}\n".format(
            '\n\n'.join(results_data)), disable_web_page_preview=1)

        if update is None:
            context.bot.send_message(
                chat_id=-1001414706781, text='Cron Job: <code>{0}</code> | @{1} notified for results'.format(chat_id, username))

    else:
        if update is not None:
            context.bot.send_message(
                chat_id, text='<u><b>No new result</b></u> 📈 has been declared since you asked last time')
